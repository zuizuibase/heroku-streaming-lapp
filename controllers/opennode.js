  ...
  try {
    if (process.env.CURRENCY !== '') {
      const charge = await opennode.createCharge({
        ...
        callback_url: process.env.CALLBACK_URL === '' ? `${process.env.HEROKU_URL}/pay/callback` : process.env.CALLBACK_URL,
        success_url: process.env.SUCCESS_URL === '' ? `${process.env.HEROKU_URL}` : process.env.SUCCESS_URL
      })
      return charge
    } else {
      const charge = await opennode.createCharge({
        description: `${process.env.LAPP_NAME}: Charge ${amount} sats`,
        amount: parseInt(amount),
        callback_url: process.env.CALLBACK_URL === '' ? '' : process.env.CALLBACK_URL,
        success_url
      })
      return charge
    }
